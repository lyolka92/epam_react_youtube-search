import {toggleLoading, updateSearchParams, updateVideos, setVideos, updateSliderParams} from "../actions/actions";
import {getVideosByKeyword} from "../../api/search-service-mock";
    ...
    return function (next) {
        return function (action) {
            if (action.type === "UPDATE_SLIDER_PARAMS") {
                ...
                action.lastAvailablePage = Math.ceil(videosArray.length / action.videosPerPage) || 1;
                action.currentPageNumber = Math.min(actionCurrentPageNumber || sliderParams.currentPageNumber, action.lastAvailablePage) || 1;

                if (videosArray.length > 0
                    && action.currentPageNumber >= action.lastAvailablePage - 2) {
                    dispatch(searchVideos(searchParams.keyword, searchParams.nextPageToken));
                }
            }
